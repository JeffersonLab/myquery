...
public class IntervalWebService extends QueryWebService {
  ...
  public <T extends Event> EventStream<T> openEventStream(
      Metadata metadata,
      boolean updatesOnly,
      Instant begin,
      Instant end,
  ...
  public Long count(Metadata metadata, boolean updatesOnly, Instant begin, Instant end)
      throws SQLException {
    return nexus.count(metadata, begin, end, updatesOnly);
  }

  public EventStream<FloatEvent> openSampleEventStream(
      String sampleType,
      Metadata<FloatEvent> metadata,
      Instant begin,
      Instant end,
      long limit,
      long count,
      boolean updatesOnly,
      boolean integrate,
      FloatEvent priorEvent,
      Class<FloatEvent> type)
    ...
    if (sampleType == null || sampleType.isEmpty()) {
      throw new IllegalArgumentException(
          "sampleType required.  Options include graphical, event, binned");
    }
      ...
      case "mysampler": // Now application-level time-based. Results in one query against database.
        ...
        if (integrate) {
          throw new UnsupportedOperationException(
              "Integration of input into mysampler algorithm has not been implemented");
        }
      case "graphical": // Application-level event-based, high graphical fidelity
      case "eventsimple": // Application-level event-based. This is likely never a good sampler
        // option given above...
        stream =
            doApplicationSampling(
                sampleType,
                metadata,
                begin,
                end,
                ...
                type);
        break;
      case "myget": // Database-level time-based.  Stored Procedure: Fastest.  "Basic" graphical
        // fidelity.  Takes first/next actual point at timed intervals
        if (integrate) {
          throw new UnsupportedOperationException(
              "Integration of input into myget sampler algorithm has not been implemented");
        }

        stream =
            doDatabaseSourceSampling(
                sampleType, metadata, updatesOnly, begin, end, limit, priorEvent, FloatEvent.class);
        break;
      default:
        throw new IllegalArgumentException(
            "Unrecognized sampleType - "
                + sampleType
                + ".  Options include graphical, eventsimple, myget, mysampler");
    }
  ...
  @SuppressWarnings("unchecked")
  private EventStream<FloatEvent> doApplicationSampling(
      String sampleType,
      Metadata metadata,
      Instant begin,
      Instant end,
    ...
    // Don't do this for mysampler as it extends BoundaryAwareStream and requires a non-null
    // priorEvent.
      ...
      case "mysampler": // Application-level event-based.  This mimics the CLI mySampler output.
        ...
        stream =
            MySamplerStream.getMySamplerStream(
                stream, begin, stepMillis, limit, priorEvent, updatesOnly, FloatEvent.class);
        break;
      default:
        throw new IllegalArgumentException(
            "Unrecognized sampleType - "
  ...
  private EventStream<FloatEvent> doDatabaseSourceSampling(
      String sampleType,
      Metadata<FloatEvent> metadata,
      boolean updatesOnly,
      Instant begin,
    ...
    if (sampleType != null && sampleType.equals("myget")) {
      // Database-level time-based.  Stored Procedure: Fastest.  "Basic" graphical fidelity.  Takes
      // first/next actual point at timed intervals
      stream = nexus.openMyGetSampleStream(metadata, begin, end, limit);
    } else {
      throw new IllegalArgumentException(
          "Unrecognized sampleType - " + sampleType + ".  Options include myget");
    }

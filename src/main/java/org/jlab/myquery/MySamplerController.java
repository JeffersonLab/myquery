...
/**
 * This method provides an end point for functionality similar to the mySampler command line
 * application.
 *
 * @author adamc
 */
...
public class MySamplerController extends QueryController {
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      // The logic around multiple streams is getting a little confusing.  If we've hit an error
      // before we process
  ...
  private boolean processChannelRequest(
    ...
    try {
      ...
      stream =
          service.openEventStream(
              metadata, begin, intervalMillis, sampleCount, updatesOnly, strategy);
      if (enumsAsStrings
          && metadata.getMyaType() == MyaDataType.DBR_ENUM
          && enumLabels != null
          && !enumLabels.isEmpty()) {

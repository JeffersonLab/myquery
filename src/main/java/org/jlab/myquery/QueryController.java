...
public class QueryController extends HttpServlet {
  ...
  public void writeIntEvent(
      String name,
      JsonGenerator gen,
      IntEvent event,
      boolean formatAsMillisSinceEpoch,
      boolean adjustMillisWithServerOffset,
  ...
  public void writeFloatEvent(
    ...
    if (!event.getCode().isDisconnection()) {
      // Round number (banker's rounding) and create String then create new BigDecimal to ensure no
      // quotes are used in JSON
  ...
  public void writeAnalyzedFloatEvent(
      String name,
      JsonGenerator gen,
    ...
    // stats should be available for even disconnect events since things like integration would be
    // happening up to
    // the disconnection.
    double[] stats = event.getEventStats();
    if (stats != null && stats.length == 1) {
      // We only support integration at this point.  Once more are supported this will have to be a
      // loop that
  ...
  public void writeLabeledEnumEvent(
      String name,
      JsonGenerator gen,
  ...
  public void writeMultiStringEvent(
      String name,
      JsonGenerator gen,
  ...
  public void writeRunningStatistics(
      String name,
      JsonGenerator gen,
   ...
   */
  public void writeEnumLabels(
      String name,
      JsonGenerator gen,
      List<ExtraInfo> enumLabelList,
      boolean formatAsMillisSinceEpoch,
      boolean adjustMillisWithServerOffset,
      DateTimeFormatter timestampFormatter) {
    if (name != null) {
  ...
  /**
   * Write a list of metadata objects as a series of JSON objects. Assumes that a JSON array has
   * been started/ended outside of this method.
   *
   ...
   * @throws IOException
   */
  public long generateIntStream(
      JsonGenerator gen,
      EventStream<IntEvent> stream,
      boolean formatAsMillisSinceEpoch,
      boolean adjustMillisWithServerOffset,
      DateTimeFormatter timestampFormatter)
      throws IOException {
  ...
  /**
   * This method write a stream of RunningStatistics associated with a given start time to a JSON
   * generator. This creates the JSON array, and does not expect one to be started outside the
   * method.
   *
   * @param name The array name
   * @param gen The JSON generator to write them to
   * @param stats The Map of timestamps to RunningStatistics that will be written to the JSON
   *     generator
   * @param timestampFormatter How to format timestamps
   * @param sigFigs The number of significant figures to round to (using Banker's rounding /
   *     HALF_UP)
   ...
   */
  public long generateStatisticsStream(
      String name,
      JsonGenerator gen,
      Map<Instant, RunningStatistics> stats,
      DateTimeFormatter timestampFormatter,
      short sigFigs,
      boolean formatAsMillisSinceEpoch,
      boolean adjustMillisWithServerOffset) {
   ...
   * @throws IOException If Unable to generate the stream
   */
  public long generateAnalyzedFloatStream(
      JsonGenerator gen,
      EventStream<AnalyzedFloatEvent> stream,
      boolean formatAsMillisSinceEpoch,
      boolean adjustMillisWithServerOffset,
      DateTimeFormatter timestampFormatter,
      short sigFigs)
   ...
   * @throws IOException
   */
  public long generateLabeledEnumStream(
      JsonGenerator gen,
      EventStream<LabeledEnumEvent> stream,
      boolean formatAsMillisSinceEpoch,
      boolean adjustMillisWithServerOffset,
      DateTimeFormatter timestampFormatter)
      throws IOException {
   ...
   * @throws IOException
   */
  public long generateMultiStringStream(
      JsonGenerator gen,
      EventStream<MultiStringEvent> stream,
      boolean formatAsMillisSinceEpoch,
      boolean adjustMillisWithServerOffset,
      DateTimeFormatter timestampFormatter)
      throws IOException {

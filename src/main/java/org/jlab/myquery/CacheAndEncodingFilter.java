 ...
 *
 * <p>This filter does not cause dynamically generated text/html to be cached - just static
 * resources like js, css, and images.
 *
 * <p>Note: By setting the Expires header you now must bust the file from the cache every time you
 * make a change to the file. A good way to do this is by adding a version string into the file path
 * or as a URL parameter.
 *
 * @author ryans
 */
@WebFilter(
    filterName = "CacheAndEncodingFilter",
    urlPatterns = {"/*"},
...
public class CacheAndEncodingFilter implements Filter {
  ...
  public static void enableCaching(HttpServletResponse response) {
    ...
    // We don't use Cache-Control max-age as it is just HTTP/1.1 replacement of HTTP/1.0 Expires,
    // but Expires still works fine.
    response.setHeader(
        "Cache-Control",
        null); // Remove header sometimes automatically added by SSL/TLS container module
    response.setHeader(
        "Pragma", null); // Remove header sometimes automatically added by SSL/TLS container module

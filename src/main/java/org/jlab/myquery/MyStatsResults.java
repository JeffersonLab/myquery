...
/**
 * A class for containing the RunningStatistics data on multiple channels over periods of time. This
 * class is intended to be used to represent the situation where a contiguous channel history has
 * been split into bins and summary statistics were calculated for each bin. This class supports
 * tracking multiple channels with different binning, but you will probably be best served by
 * keeping all of the bin sizes the same.
 *
 * @author adamc
 */
public class MyStatsResults {
   ...
   * @param timestamp The timestamp for the beginning of the time period for which the statistics
   *     are valid
